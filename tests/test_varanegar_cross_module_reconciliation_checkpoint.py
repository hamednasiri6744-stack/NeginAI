...
ROOT=Path(__file__).resolve().parents[1];B=ROOT/"scripts/windows/build_varanegar_cross_module_reconciliation_checkpoint_20260829.py";A=ROOT/"artifacts/varanegar_analysis/varanegar_cross_module_reconciliation_checkpoint_20260829.json"
...
def test_scope():assert load()["checks"]["scope"] and load()["checks"]["edges_invariants"] and load()["checks"]["schema"]
