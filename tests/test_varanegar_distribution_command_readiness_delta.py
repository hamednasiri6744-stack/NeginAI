...
ROOT=Path(__file__).resolve().parents[1];B=ROOT/"scripts/windows/build_varanegar_distribution_command_readiness_delta_20260829.py";A=ROOT/"artifacts/varanegar_analysis/varanegar_distribution_command_readiness_delta_20260829.json"
...
def test_only_distribution_changed():assert load()["delta"]["module"]=="distribution" and load()["summary"]["changed_module_count"]==1
def test_design_moves_five_to_six():
 s=load()["summary"];assert s["outcome_target_contract_module_count_before"]==5 and s["outcome_target_contract_module_count_after"]==6
