...
ROOT=Path(__file__).resolve().parents[1];B=ROOT/"scripts/windows/build_varanegar_treasury_expert_playbook_checkpoint_20260829.py";A=ROOT/"artifacts/varanegar_analysis/varanegar_treasury_expert_playbook_checkpoint_20260829.json"
...
def test_scope():assert load()["checks"]["six_playbooks"] and load()["checks"]["seven_steps"] and load()["checks"]["twelve_and_eighty_four"]
def test_runtime_zero():assert load()["checks"]["runtime_repair_zero"]
