...
ROOT=Path(__file__).resolve().parents[1];B=ROOT/"scripts/windows/build_varanegar_distribution_command_outcome_checkpoint_20260829.py";A=ROOT/"artifacts/varanegar_analysis/varanegar_distribution_command_outcome_checkpoint_20260829.json"
...
def test_gaps_pinned():assert load()["checks"]["four_commands"] and load()["checks"]["four_idempotency_gaps"] and load()["checks"]["four_transaction_gaps"]
