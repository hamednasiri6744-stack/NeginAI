...
@app.get('/')
def page(figma: bool = False):
    return HTMLResponse((root / 'warehouse-assistant.html').read_text(encoding='utf-8').replace('</body>', '<script src="/fixture.js" defer></script>' + ('<script src="https://mcp.figma.com/mcp/html-to-design/capture.js" async></script>' if figma else '') + '</body>'))
...
def fixture_script():
    return Response('''async function startWorkflowFixture(){
      ...
      initializeWorkViews();await initializeColumnLayouts();renderAutomaticPreorders();switchView('sent',false);
