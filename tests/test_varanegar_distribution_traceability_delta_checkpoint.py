...
ROOT=Path(__file__).resolve().parents[1];B=ROOT/"scripts/windows/build_varanegar_distribution_traceability_delta_checkpoint_20260829.py";A=ROOT/"artifacts/varanegar_analysis/varanegar_distribution_traceability_delta_checkpoint_20260829.json"
...
def test_scope():assert load()["checks"]["scope_counts"] and load()["checks"]["link_counts"]
def test_no_promotion():assert load()["checks"]["base_stable"] and load()["checks"]["no_promotion"]
