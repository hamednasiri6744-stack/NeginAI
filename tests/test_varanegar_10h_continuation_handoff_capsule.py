...
def test_resume_point_is_explicit_and_portable():d=load();assert d["scope"]["portable_project_root"]==ROOT.as_posix();assert d["current_resume_point"]["top_checkpoint"].endswith("varanegar_target_erp_contract_portfolio_invariant_audit_checkpoint_20260901.json");assert "settle_varanegar_analysis_chain_20260831.ps1" in d["current_resume_point"]["full_settle_command"]
