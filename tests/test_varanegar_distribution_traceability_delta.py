...
ROOT=Path(__file__).resolve().parents[1];B=ROOT/"scripts/windows/build_varanegar_distribution_traceability_delta_20260829.py";A=ROOT/"artifacts/varanegar_analysis/varanegar_distribution_traceability_delta_20260829.json"
...
def test_counts():
 s=load()["summary"];assert s["distribution_evidence_count"]==4 and s["requirement_contract_delta_count"]==7 and s["module_evidence_link_count"]==18 and s["risk_evidence_link_count"]==25
def test_existing_only():assert load()["summary"]["unique_linked_risk_count"]==9 and load()["checks"]["module_consistent"]
