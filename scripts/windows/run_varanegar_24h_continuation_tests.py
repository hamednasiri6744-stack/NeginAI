"""Run every checked-in offline Varanegar test for the active 24-hour continuation."""
...
def main() -> int:
    ...
    parser.add_argument("--exclude-wave01-bundle-bootstrap", action="store_true")
    args = parser.parse_args()
    tests = sorted((ROOT / "tests").glob("test_varanegar_*.py"))
    excluded = []
    if args.exclude_wave01_bundle_bootstrap:
        bootstrap_names = {
        ...
        for checkpoint_name in post_wave_checkpoints:
            checkpoint = json.loads((analysis / checkpoint_name).read_text(encoding="utf-8-sig"))
            bootstrap_names.update(
                Path(source["path"]).name
                for source in checkpoint.get("source_manifest", [])
                if source.get("path", "").startswith("tests/")
                and source["path"].endswith(".py")
            )
        excluded = [test for test in tests if test.name in bootstrap_names]
    ...
    output = {
        "artifact": "varanegar_24h_continuation_test_result_20260829",
        "schema_version": 1,
        ...
        "limits": [
            ...
            "The bootstrap exclusion exists only to seed the Wave-01/post-wave dependency chain; final persisted evidence must use the default complete glob.",
        ],
    }
