...
export function VisitorNeshanMap({routeId,mode,selectedCustomerId,recenterNonce,onSelectCustomer,onPrimaryCustomer,onNotice,onOrderChange}:Props){
  ...
  useEffect(()=>{if(!routeId)return;let off=false;setError('');setLeg('')
    ...
    void Promise.all([getNeshanMapConfig(),getRouteMapPlan(routeId,mode==='sales'?'sales_priority':'shortest',originForPlan)])
      .then(([c,p])=>{if(!off){setKey(c.api_key ?? '');setPlan(p);planOriginRef.current=originForPlan;orderChangeRef.current([...p.ordered_customers,...p.unlocated_customers].map(customer=>String(customer.id)));const first=routeCluster(p)[0];if(first)primaryCustomerRef.current(String(first.id))}}).catch(e=>{if(!off)setError(e instanceof Error?e.message:'Neshan map unavailable')})
    return()=>{off=true}
