...
type Props={routeId:string|null;mode:'sales'|'shortest';selectedCustomerId:string;recenterNonce:number;routeStops:VisitorRouteStop[];onSelectCustomer:(id:string)=>void;onPrimaryCustomer:(id:string)=>void;onNotice:(m:string)=>void;onOrderChange:(ids:string[])=>void;onPlan:(plan:NeshanRouteMapPlanResponse)=>void;onNavigationMetrics:(customerId:string,leg:NeshanRouteLeg|null)=>void}
...
function gpsValidationCluster(plan:NeshanRouteMapPlanResponse){const located=plan.ordered_customers.filter(c=>c.latitude!=null&&c.longitude!=null);if(located.length<=2)return located;let best=located;let bestCount=0;for(const anchor of located){const origin={latitude:Number(anchor.latitude),longitude:Number(anchor.longitude)};const near=located.filter(c=>distanceKm(origin,{latitude:Number(c.latitude),longitude:Number(c.longitude)})<=ROUTE_CLUSTER_RADIUS_KM);if(near.length>bestCount){best=near;bestCount=near.length}}return bestCount>=Math.max(2,Math.ceil(located.length*.5))?best:located}
function gpsMatchesPlan(p:Pos,plan:NeshanRouteMapPlanResponse){return gpsValidationCluster(plan).some(c=>distanceKm(p,{latitude:Number(c.latitude),longitude:Number(c.longitude)})<=MAX_ROUTE_GPS_DISTANCE_KM)}
function usableGps(p:Pos|null){return Boolean(p&&(!Number.isFinite(Number(p.accuracy))||Number(p.accuracy)<=100))}
...
export function VisitorNeshanMap({routeId,mode,selectedCustomerId,recenterNonce,routeStops,onSelectCustomer,onPrimaryCustomer,onNotice,onOrderChange,onPlan,onNavigationMetrics}:Props){
  ...
  const trustedCustomers=useMemo(()=>plan?.ordered_customers.filter(c=>c.latitude!=null&&c.longitude!=null)??[],[plan])
  ...
  useEffect(()=>{if(!routeId)return;let off=false;setError('');setLeg('')
    const originForPlan=hasTrustedPos?trustedPosRef.current:null
    void Promise.all([getNeshanMapConfig(),getRouteMapPlan(routeId,mode==='sales'?'sales_priority':'shortest',originForPlan)])
      .then(([c,p])=>{if(!off){setKey(c.api_key ?? '');setPlan(p);onPlan(p);planOriginRef.current=originForPlan;orderChangeRef.current([...p.ordered_customers,...p.unlocated_customers].map(customer=>String(customer.id)));const first=p.ordered_customers.find(customer=>customer.latitude!=null&&customer.longitude!=null);if(first)primaryCustomerRef.current(String(first.id))}}).catch(e=>{if(!off)setError(e instanceof Error?e.message:'Neshan map unavailable')})
    return()=>{off=true}
  ...
  useEffect(()=>{if(!routeId||!selected)return;routeRequestRef.current=null;routeFocusKeyRef.current='';offRouteSinceRef.current=0;followUserRef.current=true;const current=trustedPosRef.current,cached=routeLegCacheRef.current.get(String(selected.id)),cacheValid=Boolean(cached&&current&&usableGps(current)&&Date.now()-cached.at<120000&&distanceKm(cached.position,current)*1000<250),first=plan?.ordered_customers.find(c=>c.latitude!=null&&c.longitude!=null),origin=planOriginRef.current,initialValid=Boolean(plan?.has_origin&&plan?.polyline&&plan?.initial_leg&&first&&current&&origin&&usableGps(current)&&String(first.id)===String(selected.id)&&distanceKm(origin,current)*1000<80);if(cacheValid&&cached){setLeg(cached.polyline);setRouteLeg(cached.leg);onNavigationMetrics(String(selected.id),cached.leg);setRouting(false)}else if(initialValid&&plan&&current){const initialLeg=plan.initial_leg||null;setLeg(plan.polyline);setRouteLeg(initialLeg);onNavigationMetrics(String(selected.id),initialLeg);routeLegCacheRef.current.set(String(selected.id),{position:current,polyline:plan.polyline,leg:initialLeg,at:Date.now()});setRouting(false)}else{setLeg('');setRouteLeg(null);setRouting(true)}let off=false;if(!current)void geo().then(next=>{if(off)return;if(next)setPos(next);else setRouting(false)});return()=>{off=true}},[routeId,selectedCustomerId,plan,onNavigationMetrics])
  useEffect(()=>{if(!routeId||!trustedPos||!selected){setLeg('');setRouteLeg(null);setRouting(false);routeRequestRef.current=null;offRouteSinceRef.current=0;return}const key=`${routeId}:${selected.id}`,previous=routeRequestRef.current,threshold=Math.max(65,Number(trustedPos.accuracy||0)*1.5),offRoute=Boolean(leg)&&distanceToPolylineMeters(trustedPos,leg)>threshold;if(offRoute){if(!offRouteSinceRef.current)offRouteSinceRef.current=Date.now()}else{offRouteSinceRef.current=0}const sustainedOffRoute=Boolean(offRouteSinceRef.current)&&Date.now()-offRouteSinceRef.current>=5000;if(previous?.key===key&&!sustainedOffRoute)return;if(sustainedOffRoute&&Date.now()-lastRerouteRef.current<15000)return;let off=false;const shouldFocus=routeFocusKeyRef.current!==key;routeRequestRef.current={key,position:trustedPos,at:Date.now()};setRouting(true);void getRouteMapLeg(routeId,String(selected.id),trustedPos).then(r=>{if(off)return;const next=r.polyline||'';setLeg(next);setRouteLeg(r.leg||null);onNavigationMetrics(String(selected.id),r.leg||null);if(next)routeLegCacheRef.current.set(String(selected.id),{position:trustedPos,polyline:next,leg:r.leg||null,at:Date.now()});setRouting(false);offRouteSinceRef.current=0;lastRerouteRef.current=Date.now();if(next&&shouldFocus){routeFocusKeyRef.current=key;followUserRef.current=true;window.setTimeout(()=>focusPolyline(map.current,next),0)}else if(next&&sustainedOffRoute&&followUserRef.current){window.setTimeout(()=>map.current?.easeTo?.({center:[trustedPos.longitude,trustedPos.latitude],zoom:16.2,pitch:42,duration:500,essential:true}),0)}}).catch(e=>{if(off)return;setRouting(false);routeRequestRef.current=null;onNotice(e instanceof Error?e.message:'Live route unavailable')});return()=>{off=true}},[routeId,trustedPos,selected,onNotice,leg,onNavigationMetrics])
  useEffect(()=>{if(!routeId||!trustedPos||!plan||!routeLeg||routing)return;const index=plan.ordered_customers.findIndex(c=>String(c.id)===selectedCustomerId),nextCustomer=index>=0?plan.ordered_customers.slice(index+1).find(c=>c.latitude!=null&&c.longitude!=null):null;if(!nextCustomer)return;const cached=routeLegCacheRef.current.get(String(nextCustomer.id));if(cached&&Date.now()-cached.at<120000&&distanceKm(cached.position,trustedPos)*1000<250)return;let cancelled=false;const timer=window.setTimeout(()=>{void getRouteMapLeg(routeId,String(nextCustomer.id),trustedPos).then(r=>{if(cancelled)return;const next=r.polyline||'';if(next)routeLegCacheRef.current.set(String(nextCustomer.id),{position:trustedPos,polyline:next,leg:r.leg||null,at:Date.now()})}).catch(()=>undefined)},1200);return()=>{cancelled=true;window.clearTimeout(timer)}},[routeId,trustedPos,plan,selectedCustomerId,routeLeg,routing])
  ...
  return <div ref={shell} className={'vr-map-live-shell'+(fullscreen?' is-fullscreen':'')}>
    ...
    {(routing||routeLeg)?<div className="vr-map-navigation-card" dir="rtl"><div><strong>{routing?(!trustedPos?'\u062f\u0631 \u062d\u0627\u0644 \u062a\u062b\u0628\u06cc\u062a GPS\u2026':'\u062f\u0631 \u062d\u0627\u0644 \u0645\u062d\u0627\u0633\u0628\u0647 \u0645\u0633\u06cc\u0631\u2026'):(navInstruction||'\u0645\u0633\u06cc\u0631 \u0622\u0645\u0627\u062f\u0647 \u0627\u0633\u062a')}</strong><span>{selectedPlanCustomer?.store_name||selectedPlanCustomer?.name||''}</span></div><div className="vr-map-navigation-meta">{navDistance?<b>{navDistance}</b>:null}{navDuration?<b>{navDuration}</b>:null}</div></div>:null}
