...
function mapRequest(url:string,key:string){const nsh='nsh://api.neshan.org';if(url.startsWith(nsh)){const rest=url.slice(nsh.length);const sep=rest.includes('?')?'&':'?';const keyed=/(?:[?&])key=/.test(rest)?rest:rest+sep+'key='+encodeURIComponent(key);return {url:'/neshan-basemap'+keyed}}if(url.startsWith('https://api.neshan.org/'))return {url:url.replace('https://api.neshan.org','/neshan-basemap')};return {url}}
...
export function VisitorNeshanMap({routeId,mode,selectedCustomerId,recenterNonce,onSelectCustomer,onPrimaryCustomer,onNotice}:Props){
  ...
  useEffect(()=>{if(!plan||!key||!host.current)return;let dead=false,local:any=null,observer:ResizeObserver|null=null
    void loadSdk().then(sdk=>{if(dead||!host.current)return
      ...
      local=new sdk.Map({container:host.current,style:STYLE,center:trustedPos?[trustedPos.longitude,trustedPos.latitude]:first?[Number(first.longitude),Number(first.latitude)]:[51.4,35.7],zoom:trustedPos?13:10,apiKey:key,rtl:{lazy:false},transformRequest:(url:string)=>mapRequest(url,key)})
  ...
  useEffect(()=>{if(!recenterNonce)return;let off=false;void geo().then(p=>{if(off)return;if(!p){onNotice('ط·آ¯ط·آ³ط·ع¾ط·آ±ط·آ³ط؛إ’ GPS ط·آ¨ط·آ±ط¸â€ڑط·آ±ط·آ§ط·آ± ط¸â€ ط؛إ’ط·آ³ط·ع¾ ط؛إ’ط·آ§ ط¸â€¦ط¸ث†ط¸â€ڑط·آ¹ط؛إ’ط·ع¾ ط·آ¯ط·آ±ط؛إ’ط·آ§ط¸ظ¾ط·ع¾ ط¸â€ ط·آ´ط·آ¯.');return}setPos(p);if(!plan||gpsMatchesPlan(p,plan)){map.current?.flyTo({center:[p.longitude,p.latitude],zoom:15.5,duration:650,essential:true})}else{onNotice('ط¸â€¦ط¸ث†ط¸â€ڑط·آ¹ط؛إ’ط·ع¾ GPS ط·آ¨ط·آ§ ط¸â€¦ط·آ­ط·آ¯ط¸ث†ط·آ¯ط¸â€، ط¸â€¦ط·آ³ط؛إ’ط·آ± ط¸â€،ط¸â€¦ط·آ®ط¸ث†ط·آ§ط¸â€  ط¸â€ ط؛إ’ط·آ³ط·ع¾ ط¸ث† ط¸â€ ط·آ§ط·آ¯ط؛إ’ط·آ¯ط¸â€، ط¹آ¯ط·آ±ط¸ظ¾ط·ع¾ط¸â€، ط·آ´ط·آ¯.')}});return()=>{off=true}},[recenterNonce,onNotice])

  return <div className="vr-map-live-shell"><div ref={host} className="vr-map-live" aria-label="ط¸â€ ط¸â€ڑط·آ´ط¸â€، ط·آ²ط¸â€ ط·آ¯ط¸â€، ط¸â€¦ط·آ³ط؛إ’ط·آ± ط¸ظ¾ط·آ±ط¸ث†ط·آ´"/>{!plan&&!error?<span className="vr-map-live-state">ط·آ¯ط·آ± ط·آ­ط·آ§ط¸â€‍ ط·آ¨ط·آ§ط·آ±ط¹آ¯ط·آ°ط·آ§ط·آ±ط؛إ’ ط¸â€ ط¸â€ڑط·آ´ط¸â€،أ¢â‚¬آ¦</span>:null}{error?<button type="button" className="vr-map-live-state error" onClick={()=>onNotice(error)}>ط¸â€ ط¸â€ڑط·آ´ط¸â€، ط·آ¯ط·آ± ط·آ¯ط·آ³ط·ع¾ط·آ±ط·آ³ ط¸â€ ط؛إ’ط·آ³ط·ع¾</button>:null}</div>
}
