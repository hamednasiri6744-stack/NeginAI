...
export function VisitorNeshanMap({routeId,mode,selectedCustomerId,recenterNonce,onSelectCustomer,onPrimaryCustomer,onNotice,onOrderChange}:Props){
  ...
  const keyRef=useRef(''),focusTokenRef=useRef(''),routeRequestRef=useRef(null as any),routeFocusKeyRef=useRef(''),routeLegCacheRef=useRef(new Map<string,{position:Pos;polyline:string;leg:NeshanRouteLeg|null;at:number}>()),followUserRef=useRef(true),offRouteSinceRef=useRef(0),lastRerouteRef=useRef(0),needsRecoveryRef=useRef(false),retryAttemptRef=useRef(0),retryTimerRef=useRef(null as any)
  ...
  useEffect(()=>{if(!routeId||!selected)return;routeRequestRef.current=null;routeFocusKeyRef.current='';offRouteSinceRef.current=0;followUserRef.current=true;const current=trustedPosRef.current,cached=routeLegCacheRef.current.get(String(selected.id)),cacheValid=Boolean(cached&&current&&Date.now()-cached.at<120000&&distanceKm(cached.position,current)*1000<250),first=plan?.ordered_customers.find(c=>c.latitude!=null&&c.longitude!=null),initialValid=Boolean(plan?.has_origin&&plan?.polyline&&first&&String(first.id)===String(selected.id));if(cacheValid&&cached){setLeg(cached.polyline);setRouteLeg(cached.leg);setRouting(false)}else if(initialValid&&plan){setLeg(plan.polyline);setRouteLeg(plan.initial_leg||null);setRouting(false)}else{setLeg('');setRouteLeg(null);setRouting(true)}let off=false;if(!current)void geo().then(next=>{if(off)return;if(next)setPos(next);else setRouting(false)});return()=>{off=true}},[routeId,selectedCustomerId,plan])
