...
فایل `NeginAI.exe` کنار `.env` قرار دارد و با دوبارکلیک اجرا می‌شود. برنامه سرویس محلی را بالا می‌آورد و مرورگر را خودکار باز می‌کند. اگر سرویس از قبل فعال باشد، اجرای دوباره EXE فقط صفحه را باز می‌کند.

مستندات محلی در `http://127.0.0.1:8000/docs` است. برای اسکن اولیه، درخواست `POST /schema/scan` را با Header زیر ارسال کنید:
